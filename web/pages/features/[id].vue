...
const updated_at =
  feat && feat.updated_at ? new Date(feat.updated_at).toUTCString() : undefined;

useHead({
  title: feat ? "QUIC Explorer | " + feat.name : "Feat not found",
});
  ...
  <div class="feat-view view">
    ...
    <template v-if="feat">
      <UCard>
        ...
        <ul class="meta">
          <li>
            <span class="label">Long Name:</span>
            <UBadge color="gray" variant="solid">{{ feat.name || "-" }}</UBadge>
          </li>
          <li>
          ...
          <li>
            <span class="label" v-if="feat.value_meta">Value Meta:</span>
            <pre
              v-if="feat.value_meta"
              class="ring-gray-300 ring-inset ring-1 text-gray-700 bg-gray-50 rounded-md text-xs p-2"
              v-html="JSON.stringify(feat.value_meta, null, 2)"
            ></pre>
          </li>
        </ul>
        ...
        <template #footer>
          <ul class="meta">
            <li>
              <span class="label">Feature created:</span>
              <UBadge class="badge" color="gray" variant="solid">{{
                created_at || "?"
              }}</UBadge>
              by
              <UBadge class="badge" color="gray" variant="solid">{{
                feat.created_by.split("<")[0].trim() || "?"
              }}</UBadge>
            </li>
      ...
      <UCard class="mt-8">
        ...
        <UTable :rows="relevant_entries" :columns="columns">
          <template #implementation_name-data="{ row }">
            <NuxtLink :to="'/implementations/' + row.implementation_uuid">{{
